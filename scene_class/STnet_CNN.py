...
class STnet(nn.Module):
    def __init__(self, input_channel=3, num_classes=3):
        ...
        self.entry_flow_2_residual = nn.Conv2d(
            64, 128, kernel_size=1, stride=2, padding=0
        )
        ...
        self.entry_flow_3_residual = nn.Conv2d(
            128, 256, kernel_size=1, stride=2, padding=0
        )
        ...
        self.entry_flow_4_residual = nn.Conv2d(
            256, 728, kernel_size=1, stride=2, padding=0
        )
    ...
    def forward(self, x):
        ...
        entry_out2 = self.entry_flow_2(entry_out1) + self.entry_flow_2_residual(
            entry_out1
        )
        entry_out3 = self.entry_flow_3(entry_out2) + self.entry_flow_3_residual(
            entry_out2
        )
        entry_out4 = self.entry_flow_4(entry_out3) + self.entry_flow_4_residual(
            entry_out3
        )
