...
class DataAugmentation(nn.Module):
    ...
    def __init__(
        ...
        self.augs_color = nn.Sequential(
            ...
            augmentations.RandomSharpness(1.0, p=0.1),
        )

        self.augs_geom = nn.Sequential(
            augmentations.RandomThinPlateSpline(p=0.1),
            augmentations.RandomCrop((2, 2), p=0.1, cropping_mode="resample"),
        ...
        self.augs_mix = nn.Sequential(
            augmentations.RandomCutMixV2(num_mix=1, p=0.1),
        )
...
class LitClassifier(lit.LightningModule):
    def __init__(self, config="pathtoconfig"):
        ...
        self.transform = DataAugmentation(
            apply_augs_basic=True,
            apply_augs_color=True,
            apply_augs_geom=True,
            apply_augs_mix=True,
    ...
    def configure_optimizers(self):
        if self.config["parameters"]["optimizer"] == "adam":
            optimizer = torch.optim.Adam(self.parameters(), lr=float(self.learningrate))

        elif self.config["parameters"]["optimizer"] == "adamw":
            optimizer = torch.optim.AdamW(
                self.parameters(), lr=float(self.learningrate), weight_decay=1e-3
            )

        if (
            self.config["parameters"]["learningrate_sheduler"]
